...
#openImWsPort
openImWsPort=$(cat $config_path | grep openImWsPort | awk -F '[:]' '{print $NF}')
#openImApiPort
openImApiPort=$(cat $config_path | grep openImApiPort | awk -F '[:]' '{print $NF}')
#echo "port"  $openImApiPort $list3
#echo "port" $openImWsPort
