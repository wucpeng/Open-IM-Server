...
func main() {
	...
	fmt.Println("build server is starting")
	if !flag.Parsed() {
	    fmt.Println("parseFailed", os.Args[1:])
	    flag.Parse()
	}
    fmt.Println("build server is starting 1", *logLevel, *openIMApiPort, *sdkWsPort, *openIMWsPort, constant.LogFileName)
	...
	case "linux":
		...
		fmt.Println("build linux server is starting", sdkWsPort, utils.ServerIP)
