...
func (mc *OnlineHistoryMongoConsumerHandler) handleChatWs2Mongo(cMsg *sarama.ConsumerMessage, msgKey string, _ sarama.ConsumerGroupSession) {
	...
	err = db.DB.BatchInsertChat2Mongo(msgFromMQ.AggregationID, msgFromMQ.MessageList, msgFromMQ.TriggerID, msgFromMQ.LastSeq)
	if err != nil {
	...
	for _, v := range msgFromMQ.MessageList {
		if v.MsgData.ContentType == constant.DeleteMessageNotification {
			...
			//log.Info(msgFromMQ.TriggerID, "handleChatWs2Mongo AdvancedRevoke: ", seqMsg, err)
			if err == nil && seqMsg != nil {
...
func (och *OnlineHistoryMongoConsumerHandler) ConsumeClaim(sess sarama.ConsumerGroupSession,
	...
	log.Info("", "online new session msg come", claim.HighWaterMarkOffset(), claim.Topic(), claim.Partition())
	for msg := range claim.Messages() {
		log.Info("", "kafka get info to mongo", "msgTopic", msg.Topic, "msgPartition", msg.Partition,  "key", string(msg.Key))
		if len(msg.Value) != 0 {
