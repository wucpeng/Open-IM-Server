...
func (rpc *rpcChat) SendMsg(_ context.Context, pb *pbChat.SendMsgReq) (*pbChat.SendMsgResp, error) {
	...
	case constant.GroupChatType:
		...
		if callbackResp.GlobalShield == 1 {
			memberUserIDList = []string{pb.MsgData.SendID} //全局禁言 或群禁言 只发送给自己
		} else { //计算群 禁言
			opInfo, err := imdb.GetGroupMemberInfoByGroupIDAndUserID(pb.MsgData.GroupID, pb.MsgData.SendID)
			if err != nil {
				log.Error(pb.OperationID, utils.GetSelfFuncName(), "GetGroupMemberInfoByGroupIDAndUserID: ", err.Error())
			} else {
				var MuteEndTime uint32 = uint32(opInfo.MuteEndTime.Unix())
				if MuteEndTime > uint32(time.Now().Unix()) {
					memberUserIDList = []string{pb.MsgData.SendID} //群禁言 只发送给自己
				}
...
func (rpc *rpcChat) sendMsgToWriter(m *pbChat.MsgDataToMQ, key string, status string) error {
	//log.NewError(m.OperationID, "sendMsgToWriter", status, key, m.String())
	switch status {
...
func modifyMessageByUserMessageReceiveOpt(userID, sourceID string, sessionType int, pb *pbChat.SendMsgReq) bool {
	if userID == pb.MsgData.SendID { //发送者是自己
		return true
	}
	...
	//log.NewError(pb.OperationID, "modifyMessageByUserMessageReceiveOpt", userID, opt)
	switch opt {
	...
	//log.NewError(pb.OperationID, "modifyMessageByUserMessageReceiveOpt", userID, singleOpt, conversationID)
	switch singleOpt {
	case constant.ReceiveMessage:
		return true
	case constant.NotReceiveMessage:
...
func (rpc *rpcChat) sendMsgToGroupOptimization(list []string, groupPB *pbChat.SendMsgReq, status string, sendTag *bool, wg *sync.WaitGroup) {
	...
	for _, v := range list {
		...
		log.Info(groupPB.OperationID, "sendMsgToGroupOptimization", v, isSend)
		if isSend {
