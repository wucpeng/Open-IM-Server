	...
	if req.Conversation.ConversationType == constant.SingleChatType && req.FieldType == constant.FieldIsPrivateChat {
		...
		if isSyncConversation {
			for _, v := range req.UserIDList {
				...
				//chat.ConversationChangeNotification(req.OperationID, v) //deleted by wg 2022-12-27
			}
		} else {
			for _, v := range req.UserIDList {
				if err = rocksCache.DelConversationFromCache(v, req.Conversation.ConversationID); err != nil {
				...
				//chat.ConversationUnreadChangeNotification(req.OperationID, v, req.Conversation.ConversationID, conversation.UpdateUnreadCountTime) //deleted by wg 2022-12-27
			}
		}
